...
if(getRversion() >= "2.15.1") utils::globalVariables(c('bag_model','pred.bag','gg_pr_Curve','gg_Cumulative_curve','gg_roc_curve','gg_lift_curve'))
...
shinyServer(function(input, output) {
  ...
  output$summary <- renderPrint({
    ...
    pred.bag <<- function(m,d) {
      p <- predict(m,d,type='probability')
      data.frame(cl=colnames(p)[apply(p,1,which.max)],p=apply(p,1,max)
      )
    }

    bag_model <<- SelfTrain(yuqi~.,train_data,learner('Bagging',list(control=Weka_control(O=TRUE,I=input$ntree))),'pred.bag')
  ...
  output$pr_curve <- renderPlot({
    ...
    n1 <- NCOL(train_data)
    preds <- predict(bag_model,test_data[,1:(n1-1)],type = 'probability')
    or <- order(preds[,'fraud'],decreasing = T)
    labels <- test_data[or,n1]
  ...
  output$downloadReport <- downloadHandler(
    filename = function() {
      paste('BaggingReport', sep = '.', switch(
...
plot_gg_lift_curve()
plot_grid()
```       ',file='BaggingReport.Rmd',append=F)

      out <- render('BaggingReport.Rmd', switch(
        input$format,
  ...
  plot_grid <- function(){
    grid.arrange(gg_pr_Curve, gg_Cumulative_curve, gg_roc_curve, gg_lift_curve, ncol=2, nrow=2, widths=c(2,2), heights=c(2,2),main='Self-Traing Based on Bagging')
  }
